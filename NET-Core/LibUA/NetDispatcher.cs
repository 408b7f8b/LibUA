        ...
        public class NetDispatcher : NetDispatcherBase
        {
            ...
            protected override int Consume(MemoryBuffer recvBuf)
            {
                    ...
                    messageType == (uint)MessageType.Close)
                {
                    ...
                    {
                        logger?.Log(LogLevel.Error, "{LoggerID}: Message type 0x{messageType:X} is not supported before SecurityPolicy is set", LoggerID(), messageType);
            ...
            protected int DispatchMessage(SLChannel config, MemoryBuffer recvBuf)
            {
                ...
                {
                    logger?.Log(LogLevel.Error, "{LoggerID}: Requested security token ID {securityTokenId} but current is {tokenId} and previous was {prevTokenId}", LoggerID(), securityTokenId, config.TokenID, config.PrevTokenID);
                ...
                {
                    logger?.Log(LogLevel.Error, "{LoggerID}: Sequence number is {remoteSequence}, expected {sequenceNumber} or higher", LoggerID(), securitySeqNum, config.RemoteSequence.SequenceNumber);
                ...
                logger?.Log(LogLevel.Information, "{LoggerID}: Message type {messageType} with SL state {SLState}", LoggerID(), typeId, config.SLState);
                ...
                logger?.Log(LogLevel.Error, "{LoggerID}: Message type {messageType} is not supported with SL state {SLState}", LoggerID(), typeId, config.SLState);
            ...
            private void DispatchMessage_SecureAndSend(SLChannel config, MemoryBuffer respBuf)
            {
                ...
                if (numChunks > 1)
                {
                    ...
                    //var bigChunkBuffer = new MemoryBuffer((int)config.TL.Config.MaxMessageSize);
            ...
            protected int DispatchHello(SLChannel config, MemoryBuffer recvBuf)
            {
                ...
                //	if (logger != null)
                //	{
                //		logger.Log(LogLevel.Error, "{LoggerID}: Hello expected {messageSize} bytes, parsed {parsedBytes}", LoggerID(), messageSize, recvBuf.Position));
                //	}
                //}
